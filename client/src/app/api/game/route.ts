...
export async function GET(request : NextRequest) {
  ...
  let body = `
    search "${query}";
    fields id, name, summary, cover.image_id, first_release_date; where version_parent = null & category = 0;
    offset ${page*perPage};
    limit ${perPage};
    ${order ? `order ${order};` : ""}
  `;
  ...
  results.forEach((item : any) => {
    ...
    if (item.first_release_date) {
      item.first_release_date = moment.unix(item.first_release_date).format("YYYY"); }
  });

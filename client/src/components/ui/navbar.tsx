...
export default function Navbar() {
    ...
    return (
        <nav className="w-screen h-16 m-0 flex bg-scale-1000 text-scale-0 items-center p-12 justify-between gap-5 fixed top-0 z-10 drop-shadow-md">
            <a href="/" className="flex-shrink-0"><img className="purple" src="/images/logo.svg"/></a>
            <form className="w-100 flex grow mx-5" action="/search">
                <input id="query" name="query" type="text" className="input-box rounded-r-none w-0" required={true} defaultValue={query}/>

        ...
        <nav className="w-screen h-16 m-0 flex bg-scale-1000 text-scale-0 items-center justify-between fixed top-0 z-10 drop-shadow-md sm:gap-5 gap-2 sm:py-12 sm:px-8 py-10 px-6">
            ...
            <form className="flex min-w-5 grow" action="/search">
                ...
                <div className="bg-scale-800 text-scale-0 min-w-0 w-18 sm:w-24 shrink-0">
                    <select name="type" hidden={active && width < cutoff} className="bg-scale-800 text-scale-0 w-full h-full sm:px-3 px-2" defaultValue={type} onChange={(e) => setSelection(e.target.value)}>
